...
def _strength_clause(cand: dict, scored: dict) -> str:
    ...
    # Pick the genuinely dominant strength to lead with (variation by candidate).
    if comp["role"] >= 0.9 and core:
        return (f"{title} with {yoe:.0f} yrs; direct retrieval/ranking signal "
                f"({', '.join(core)})")
    if comp["role"] >= 0.9:
        return f"{title} with {yoe:.0f} yrs in applied-ML/retrieval roles"
...
def _concern_clause(cand: dict, scored: dict) -> str:
    ...
    if isinstance(np, int) and np >= 90:
        concerns.append(f"long notice period ({np}d)")
